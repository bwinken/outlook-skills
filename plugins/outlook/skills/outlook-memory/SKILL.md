...
1. Ask with **AskUserQuestion** (never a plain text question):
...
3. On "build and scan":
   ...
   - Show the draft as a table (title, category, tags, one-line content) and ask with **AskUserQuestion**: 「全部寫入」/「讓我挑」/「取消」. For "let me pick", ask a multiSelect question listing the titles.
