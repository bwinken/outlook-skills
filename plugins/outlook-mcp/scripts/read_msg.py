...
def run(a):
    """Parse every file in a.files; returns a list with one dict per file."""
    results = []
    for path in a.files:
        ...
        if ext == ".msg":
            r = parse_msg(path, a.headers, a.extract_to)
        elif ext == ".eml":
            r = parse_eml(path, a.headers, a.extract_to)
        else:
            raise SystemExit(f"Unsupported extension '{ext}': {path} (expected .msg or .eml)")
        if a.max_body and len(r["body"]) > a.max_body:
            r["body"] = r["body"][: a.max_body] + "\n[... truncated ...]"
